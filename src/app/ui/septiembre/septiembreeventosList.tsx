...
import { SeptiembreEventos, defaultSeptiembreValues } from "@/lib/types";
...
export const SeptiembreEventosList = () => {
  const { register, handleSubmit, reset } = useForm<SeptiembreEventos>({
    defaultValues: defaultSeptiembreValues,
    resolver: zodResolver(SeptiembreEventosSchema),
  });

  const [modalVisible, setModalVisible] = useState(false);
  const [view, setView] = useState<boolean>(false);
  const [button, setButton ] = useState<boolean>(true);
  const [selectedEventDetails, setSelectedEventDetails] = useState<SeptiembreEventos | null>(null);
  const [selectedEvent, setSelectedEvent] = useState<SeptiembreEventos | null>(null);
  let back = '/calendario'
  const eventos = trpc.septiembre.readAll.useQuery();
  ...
  const crearEvento = trpc.septiembre.create.useMutation({
    onSuccess: () => {
      eventos.refetch();
      reset();
    },
  ...
  const updateSeptiembre = trpc.septiembre.update.useMutation({
    onSuccess: async () => {
      eventos.refetch();
      reset();
    },
  ...
  const deleteSeptiembre = trpc.septiembre.delete.useMutation({
    onSuccess: () => {
      eventos.refetch();
    },
  });

  const onSubmit = (data: SeptiembreEventos) => {
    if (data.id) {
      updateSeptiembre.mutate(data);
      toggleButtonAndView()
    } else {
  ...
  return (
    <div className="eventos-mensuales-container">
      <h1>Formulario ingreso de eventos septiembre</h1>
    ...
    <input placeholder="Default" defaultValue={'septiembre'} type="text" {...register("mes")} /> 
    </div>
    <br />       
    ...
    <div className="input-format">
    <label htmlFor="mes">mes</label>
    <input placeholder="Default" defaultValue={'septiembre'} {...register("mes")} /> 
    </div>
    <br />       
    <button type="submit">Ingresar</button>
  </form>
      <br />
      <h2 className="mb-5">Eventos septiembre</h2>
        ...
        {eventos.isLoading ? (
          ...
          eventos.data?.map((evento) => (            
            <li className="maxw" key={evento.id}>
              ...
              | <button onClick={() => deleteSeptiembre.mutate(evento)}>
