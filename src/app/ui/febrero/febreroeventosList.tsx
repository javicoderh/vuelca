...
import { FebreroEventos, defaultEneroValues } from "@/lib/types";
...
export const FebreroEventosList = () => {
  const { register, handleSubmit, reset } = useForm<FebreroEventos>({
    defaultValues: defaultEneroValues,
    resolver: zodResolver(FebreroEventosSchema),
  });
  ...
  const [selectedEventDetails, setSelectedEventDetails] = useState<FebreroEventos | null>(null);

  const [selectedEvent, setSelectedEvent] = useState<FebreroEventos | null>(null);
  let back = '/calendario'
  const eventos = trpc.febrero.readAll.useQuery();
  ...
  const crearEvento = trpc.febrero.create.useMutation({
    onSuccess: () => {
      eventos.refetch();
      reset();
    },
  ...
  const updateFebrero = trpc.febrero.update.useMutation({
    onSuccess: async () => {
      eventos.refetch();
      reset();
    },
  ...
  const deleteFebrero = trpc.febrero.delete.useMutation({
    onSuccess: () => {
      eventos.refetch();
    },
  });

  const onSubmit = (data: FebreroEventos) => {
    if (data.id) {
      updateFebrero.mutate(data);
      toggleButtonAndView()
    } else {
  ...
  return (
    <div className="eventos-mensuales-container">
      <h1>Formulario ingreso de eventos febrero:</h1>
    ...
    <input placeholder="Default" defaultValue={'febrero'} type="text" {...register("mes")} /> 
    </div>
    <br />       
    ...
    <div className="input-format">
    <label htmlFor="mes">mes</label>
    <input placeholder="Default" defaultValue={'febrero'} {...register("mes")} /> 
    </div>
    <br />       
    <button type="submit">Ingresar</button>
  </form>
      ...
      <br />
      <h2 className="mb-5">Eventos febrero:</h2>
        ...
        {eventos.isLoading ? (
          ...
          eventos.data?.map((evento) => (            
            <li className="maxw" key={evento.id}>
              ...
              | <button onClick={() => deleteFebrero.mutate(evento)}>
