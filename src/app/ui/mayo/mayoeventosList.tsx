...
import { MayoEventos, defaultMayoValues } from "@/lib/types";
...
export const MayoEventosList = () => {
  const { register, handleSubmit, reset } = useForm<MayoEventos>({
    defaultValues: defaultMayoValues,
    resolver: zodResolver(MayoEventosSchema),
  });
  ...
  let back = '/calendario'
  const eventos = trpc.mayo.readAll.useQuery();
  ...
  const crearEvento = trpc.mayo.create.useMutation({
    onSuccess: () => {
      eventos.refetch();
      reset();
    },
  ...
  const updateMayo = trpc.mayo.update.useMutation({
    onSuccess: async () => {
      eventos.refetch();
      reset();
    },
  ...
  const deleteMayo = trpc.mayo.delete.useMutation({
    onSuccess: () => {
      eventos.refetch();
    },
  });

  const onSubmit = (data: MayoEventos) => {
    if (data.id) {
      updateMayo.mutate(data);
      toggleButtonAndView()
    } else {
  ...
  return (
    <div className="eventos-mensuales-container">
      <h1>Formulario ingreso de eventos mayo</h1>
    ...
    <input placeholder="Default" defaultValue={'mayo'} type="text" {...register("mes")} /> 
    </div>
    <br />       
    ...
    <div className="input-format">
    <label htmlFor="mes">mes</label>
    <input placeholder="Default" defaultValue={'mayo'} {...register("mes")} /> 
    </div>
    <br />       
    <button type="submit">Ingresar</button>
  </form>
      ...
      <br />
      <h2 className="mb-5">Eventos mayo</h2>
        ...
        {eventos.isLoading ? (
          ...
          eventos.data?.map((evento) => (            
            <li className="maxw" key={evento.id}>
              ...
              | <button onClick={() => deleteMayo.mutate(evento)}>
