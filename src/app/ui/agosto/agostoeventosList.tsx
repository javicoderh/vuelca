...
import { AgostoEventos, defaultAgostoValues } from "@/lib/types";
...
export const AgostoEventosList = () => {
  const { register, handleSubmit, reset } = useForm<AgostoEventos>({
    defaultValues: defaultAgostoValues,
    resolver: zodResolver(AgostoEventosSchema),
  });

  const [modalVisible, setModalVisible] = useState(false);
  const [view, setView] = useState<boolean>(false);
  const [button, setButton ] = useState<boolean>(true);
  const [selectedEventDetails, setSelectedEventDetails] = useState<AgostoEventos | null>(null);
  const [selectedEvent, setSelectedEvent] = useState<AgostoEventos | null>(null);
  let back = '/calendario'
  const eventos = trpc.agosto.readAll.useQuery();
  ...
  const crearEvento = trpc.agosto.create.useMutation({
    onSuccess: () => {
      eventos.refetch();
      reset();
    },
  ...
  const updateAgosto = trpc.agosto.update.useMutation({
    onSuccess: async () => {
      eventos.refetch();
      reset();
    },
  ...
  const deleteAgosto = trpc.agosto.delete.useMutation({
    onSuccess: () => {
      eventos.refetch();
    },
  });

  const onSubmit = (data: AgostoEventos) => {
    if (data.id) {
      updateAgosto.mutate(data);
      toggleButtonAndView()
    } else {
  ...
  return (
    <div className="eventos-mensuales-container">
      <h1>Formulario ingreso de eventos agosto</h1>
    ...
    <input placeholder="Default" defaultValue={'agosto'} type="text" {...register("mes")} /> 
    </div>
    <br />       
    ...
    <div className="input-format">
    <label htmlFor="mes">mes</label>
    <input placeholder="Default" defaultValue={'agosto'} {...register("mes")} /> 
    </div>
    <br />       
    <button type="submit">Ingresar</button>
  </form>
      <br />
      <h2 className="mb-5">Eventos agosto</h2>
        ...
        {eventos.isLoading ? (
          ...
          eventos.data?.map((evento) => (            
            <li className="maxw" key={evento.id}>
              ...
              | <button onClick={() => deleteAgosto.mutate(evento)}>
