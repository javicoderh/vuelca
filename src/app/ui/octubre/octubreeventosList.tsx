...
import { OctubreEventos, defaultOctubreValues } from "@/lib/types";
...
export const OctubreEventosList = () => {
  const { register, handleSubmit, reset } = useForm<OctubreEventos>({
    defaultValues: defaultOctubreValues,
    resolver: zodResolver(OctubreEventosSchema),
  });

  const [modalVisible, setModalVisible] = useState(false);
  const [view, setView] = useState<boolean>(false);
  const [button, setButton ] = useState<boolean>(true);
  const [selectedEventDetails, setSelectedEventDetails] = useState<OctubreEventos | null>(null);
  const [selectedEvent, setSelectedEvent] = useState<OctubreEventos | null>(null);
  let back = '/calendario'
  const eventos = trpc.octubre.readAll.useQuery();
  ...
  const crearEvento = trpc.octubre.create.useMutation({
    onSuccess: () => {
      eventos.refetch();
      reset();
    },
  ...
  const updateOctubre = trpc.octubre.update.useMutation({
    onSuccess: async () => {
      eventos.refetch();
      reset();
    },
  ...
  const deleteOctubre = trpc.octubre.delete.useMutation({
    onSuccess: () => {
      eventos.refetch();
    },
  });

  const onSubmit = (data: OctubreEventos) => {
    if (data.id) {
      updateOctubre.mutate(data);
      toggleButtonAndView()
    } else {
  ...
  return (
    <div className="eventos-mensuales-container">
      <h1>Formulario ingreso de eventos octubre</h1>
    ...
    <input placeholder="Default" defaultValue={'octubre'} type="text" {...register("mes")} /> 
    </div>
    <br />       
    ...
    <div className="input-format">
    <label htmlFor="mes">mes</label>
    <input placeholder="Default" defaultValue={'octubre'} {...register("mes")} /> 
    </div>
    <br />       
    <button type="submit">Ingresar</button>
  </form>
      <br />
      <h2 className="mb-5">Eventos octubre</h2>
        ...
        {eventos.isLoading ? (
          ...
          eventos.data?.map((evento) => (            
            <li className="maxw" key={evento.id}>
              ...
              | <button onClick={() => deleteOctubre.mutate(evento)}>
