...
function ProximosEventos() {
  ...
  return (
    <div className='proximosEventos hidden md:grid'>
      <h2 className='pl-[5px] mb-[25px] text-gray-700'>eventos febrero</h2>
      <ul className='proximos-eventos-ul'>
        {eventos.map((evento) => (
          <Link key={evento.nombre} href={'/'+evento.nombre}>
            <li>
              <h2 className='p-[3px] text-gray-500'>{evento.nombre}:</h2>
              <div className='flex flex-row bg-gray-700 text-gray-500'>
                <h3 className='pl-[6px] text-gray-500'>{evento.mes} - {evento.fecha}</h3>
