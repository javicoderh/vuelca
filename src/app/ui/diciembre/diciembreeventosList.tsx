...
import { DiciembreEventos, defaultDiciembreValues } from "@/lib/types";
...
export const DiciembreEventosList = () => {
  const { register, handleSubmit, reset } = useForm<DiciembreEventos>({
    defaultValues: defaultDiciembreValues,
  ...
  const [selectedEventDetails, setSelectedEventDetails] = useState<DiciembreEventos | null>(null);
  const [selectedEvent, setSelectedEvent] = useState<DiciembreEventos | null>(null);
  let back = '/calendario'
  const eventos = trpc.diciembre.readAll.useQuery();
  ...
  const crearEvento = trpc.diciembre.create.useMutation({
    onSuccess: () => {
      eventos.refetch();
      reset();
    },
  ...
  const updateDiciembre = trpc.diciembre.update.useMutation({
    onSuccess: async () => {
      eventos.refetch();
      reset();
    },
  ...
  const deleteDiciembre = trpc.diciembre.delete.useMutation({
    onSuccess: () => {
      eventos.refetch();
    },
  });

  const onSubmit = (data: DiciembreEventos) => {
    if (data.id) {
      updateDiciembre.mutate(data);
      toggleButtonAndView()
    } else {
  ...
  return (
    ...
    <input placeholder="Default" defaultValue={'diciembre'} type="text" {...register("mes")} /> 
    </div>
    <br />       
    ...
    <div className="input-format">
    <label htmlFor="mes">mes</label>
    <input placeholder="Default" defaultValue={'diciembre'} {...register("mes")} /> 
    </div>
    <br />       
    <button type="submit">Ingresar</button>
  </form>
        ...
        {eventos.isLoading ? (
          ...
          eventos.data?.map((evento) => (            
            <li className="maxw" key={evento.id}>
              ...
              | <button onClick={() => deleteDiciembre.mutate(evento)}>
