...
import { JulioEventos, defaultJulioValues } from "@/lib/types";
...
export const JulioEventosList = () => {
  const { register, handleSubmit, reset } = useForm<JulioEventos>({
    defaultValues: defaultJulioValues,
    resolver: zodResolver(JulioEventosSchema),
  });
  ...
  const [selectedEventDetails, setSelectedEventDetails] = useState<JulioEventos | null>(null);
  const [selectedEvent, setSelectedEvent] = useState<JulioEventos | null>(null);
  let back = '/calendario'
  const eventos = trpc.julio.readAll.useQuery();
  ...
  const updateJulio = trpc.julio.update.useMutation({
    onSuccess: async () => {
      eventos.refetch();
      reset();
    },
  ...
  const deleteJulio = trpc.enero.delete.useMutation({
    onSuccess: () => {
      eventos.refetch();
    },
  });

  const onSubmit = (data: JulioEventos) => {
    if (data.id) {
      updateJulio.mutate(data);
      toggleButtonAndView()
    } else {
  ...
  return (
    <div className="eventos-mensuales-container">
      <h1>Formulario ingreso de eventos julio</h1>
    ...
    <input placeholder="Default" defaultValue={'julio'} type="text" {...register("mes")} /> 
    </div>
    <br />       
    ...
    <div className="input-format">
    <label htmlFor="mes">mes</label>
    <input placeholder="Default" defaultValue={'julio'} {...register("mes")} /> 
    </div>
    <br />       
    <button type="submit">Ingresar</button>
  </form>
      ...
      <br />
      <h2 className="mb-5">Eventos julio</h2>
        ...
        {eventos.isLoading ? (
          ...
          eventos.data?.map((evento) => (            
            <li className="maxw" key={evento.id}>
              ...
              | <button onClick={() => deleteJulio.mutate(evento)}>
