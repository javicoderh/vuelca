...
import { NoviembreEventos, defaultNoviembreValues } from "@/lib/types";
...
export const NoviembreEventosList = () => {
  const { register, handleSubmit, reset } = useForm<NoviembreEventos>({
    defaultValues: defaultNoviembreValues,
    resolver: zodResolver(NoviembreEventosSchema),
  });
  ...
  const crearEvento = trpc.noviembre.create.useMutation({
    onSuccess: () => {
      eventos.refetch();
      reset();
    },
  ...
  const updateNoviembre = trpc.noviembre.update.useMutation({
    onSuccess: async () => {
      eventos.refetch();
      reset();
    },
  ...
  const deleteNoviembre = trpc.noviembre.delete.useMutation({
    onSuccess: () => {
      eventos.refetch();
    },
  });

  const onSubmit = (data: NoviembreEventos) => {
    if (data.id) {
      updateNoviembre.mutate(data);
      toggleButtonAndView()
    } else {
  ...
  return (
    <div className="eventos-mensuales-container">
      <h1>Formulario ingreso de eventos noviembre</h1>
    ...
    <input placeholder="Default" defaultValue={'noviembre'} type="text" {...register("mes")} /> 
    </div>
    <br />       
    ...
    <div className="input-format">
    <label htmlFor="mes">mes</label>
    <input placeholder="Default" defaultValue={'noviembre'} {...register("mes")} /> 
    </div>
    <br />       
    <button type="submit">Ingresar</button>
  </form>
      <br />
      <h2 className="mb-5">Eventos noviembre</h2>
        ...
        {eventos.isLoading ? (
          ...
          eventos.data?.map((evento) => (            
            <li className="maxw" key={evento.id}>
              ...
              | <button onClick={() => deleteNoviembre.mutate(evento)}>
