...
import { NoviembreEventos } from "@/lib/types";
...
const NoviembreInteractivo = () => {
  const [modalVisible, setModalVisible] = useState(false);
  const [selectedEvent, setSelectedEvent] = useState<NoviembreEventos[] | null>(null);
  const back = '/calendario';
  const eventos = trpc.noviembre.readAll.useQuery()?.data as NoviembreEventos[] | undefined;
  return (
      <div className="mes-interactivo">
        <h2 className="text-white">noviembre</h2>      
        <button
          className="mr-3"
          onClick={() => {
...
export default NoviembreInteractivo