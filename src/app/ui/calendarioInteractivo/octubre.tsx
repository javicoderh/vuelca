...
import { OctubreEventos } from "@/lib/types";
...
const OctubreInteractivo = () => {
  const [modalVisible, setModalVisible] = useState(false);
  const [selectedEvent, setSelectedEvent] = useState<OctubreEventos[] | null>(null);
  const back = '/calendario';
  const eventos = trpc.octubre.readAll.useQuery()?.data as OctubreEventos[] | undefined;
  return (
      <div className="mes-interactivo">
        <h2 className="text-white">octubre</h2>      
        <button
          className="mr-3"
          onClick={() => {
...
export default OctubreInteractivo