...
import { AgostoEventos } from "@/lib/types";
...
const AgostoInteractivo = () => {
  const [modalVisible, setModalVisible] = useState(false);
  const [selectedEvent, setSelectedEvent] = useState<AgostoEventos[] | null>(null);
  const back = '/calendario';
  const eventos = trpc.agosto.readAll.useQuery()?.data as AgostoEventos[] | undefined;
  return (
      <div className="mes-interactivo">
        <h2 className="text-white">agosto</h2>      
        <button
          className="mr-3"
          onClick={() => {
...
export default AgostoInteractivo