...
import { SeptiembreEventos } from "@/lib/types";
...
const SeptiembreInteractivo = () => {
  const [modalVisible, setModalVisible] = useState(false);
  const [selectedEvent, setSelectedEvent] = useState<SeptiembreEventos[] | null>(null);
  const back = '/calendario';
  const eventos = trpc.septiembre.readAll.useQuery()?.data as SeptiembreEventos[] | undefined;
  return (
      <div className="mes-interactivo">
        <h2 className="text-white">septiembre</h2>      
        <button
          className="mr-3"
          onClick={() => {
...
export default SeptiembreInteractivo