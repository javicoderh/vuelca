...
import { FebreroEventos } from "@/lib/types";
...
const FebreroInteractivo = () => {
  const [modalVisible, setModalVisible] = useState(false);
  const [selectedEvent, setSelectedEvent] = useState<FebreroEventos[] | null>(null);
  const back = '/calendario';
  const eventos = trpc.febrero.readAll.useQuery()?.data as FebreroEventos[] | undefined;

  return (
      <div className="mes-interactivo">
        <h2 className="text-white">febrero</h2>      
        <button
          className="mr-3"
          onClick={() => {
...
export default FebreroInteractivo