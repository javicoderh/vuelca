...
import { DiciembreEventos } from "@/lib/types";
...
const DiciembreInteractivo = () => {
  const [modalVisible, setModalVisible] = useState(false);
  const [selectedEvent, setSelectedEvent] = useState<DiciembreEventos[] | null>(null);
  const back = '/calendario';
  const eventos = trpc.diciembre.readAll.useQuery()?.data as DiciembreEventos[] | undefined;
  return (
      <div className="mes-interactivo">
        <h2 className="text-white">diciembre</h2>      
        <button
          className="mr-3"
          onClick={() => {
...
export default DiciembreInteractivo